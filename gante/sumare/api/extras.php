<?php
// ============================================
// GANTE SUMARE — API de Extras (Tamanhos, Caixas, Toppings, Sabores do Dia)
// ============================================
...
function handleGet() {
    ...
    if ($table === 'flavors_of_the_day') {
        ...
        try {
            $stmt = $db->query("
                SELECT p.* FROM {$prefix}products p
                INNER JOIN {$prefix}flavors_of_the_day f ON p.id = f.product_id
                WHERE p.type = 'gelato'
                ORDER BY p.name ASC
            ");
...
function handlePost() {
    ...
    if ($table === 'flavors_of_the_day') {
        ...
        $db->exec("
            CREATE TABLE IF NOT EXISTS {$prefix}flavors_of_the_day (
        ...
        $db->exec("DELETE FROM {$prefix}flavors_of_the_day");

        if (!empty($productIds)) {
            $stmt = $db->prepare("INSERT INTO {$prefix}flavors_of_the_day (product_id) VALUES (:pid)");
