<?php
// ============================================
// GANTE ORLANDIA — Setup: Criar Tabelas e Seed de Dados
// ============================================
//
// Execute este script UMA VEZ para configurar o banco.
// Acesse: https://seudominio.com/orlandia/api/setup.php?token=gante2026setup
//
...
try {
    ...
    ");
    $results[] = "Tabela \"{$prefix}categories\" criada.";

    // Corrigir tabelas existentes: trocar ENUM por VARCHAR se necessario
    try {
        $db->exec("ALTER TABLE {$prefix}categories MODIFY COLUMN type VARCHAR(50) NOT NULL");
        $results[] = "Coluna \"type\" da tabela {$prefix}categories atualizada para VARCHAR(50).";
    ...
    ");
    $results[] = "Tabela \"{$prefix}products\" criada.";

    // Corrigir products type tambem
    try {
        $db->exec("ALTER TABLE {$prefix}products MODIFY COLUMN type VARCHAR(50) NOT NULL");
        $results[] = "Coluna \"type\" da tabela {$prefix}products atualizada para VARCHAR(50).";
    } catch (PDOException $e) {
        // Ignorar
    }

    $db->exec("
        CREATE TABLE IF NOT EXISTS {$prefix}gelato_sizes (
    ...
    ");
    $results[] = "Tabela \"{$prefix}gelato_sizes\" criada.";

    $db->exec("
        CREATE TABLE IF NOT EXISTS {$prefix}chocolate_boxes (
            id VARCHAR(50) PRIMARY KEY,
            name VARCHAR(100) NOT NULL,
    ...
    ");
    $results[] = "Tabela \"{$prefix}chocolate_boxes\" criada.";

    $db->exec("
        CREATE TABLE IF NOT EXISTS {$prefix}toppings (
            id VARCHAR(50) PRIMARY KEY,
            name VARCHAR(100) NOT NULL,
            price DECIMAL(10,2) NOT NULL DEFAULT 0.00
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
    ");
    $results[] = "Tabela \"{$prefix}toppings\" criada.";

    // ========== LIMPAR DADOS ANTIGOS (para re-execucao segura) ==========

    $db->exec("DELETE FROM {$prefix}products");
    $db->exec("DELETE FROM {$prefix}categories");
    $db->exec("DELETE FROM {$prefix}gelato_sizes");
    $db->exec("DELETE FROM {$prefix}chocolate_boxes");
    $db->exec("DELETE FROM {$prefix}toppings");
    $db->exec("ALTER TABLE {$prefix}products AUTO_INCREMENT = 1");
    $db->exec("ALTER TABLE {$prefix}categories AUTO_INCREMENT = 1");
    $results[] = 'Dados antigos removidos (re-execucao segura).';

    // ========== SEED: CATEGORIAS ==========

    $catStmt = $db->prepare("INSERT INTO {$prefix}categories (name, type) VALUES (:name, :type)");
    ...
    $results[] = count($allCats) . ' categorias inseridas.';

    // Mapear IDs
    $catMap = [];
    $rows = $db->query("SELECT id, name, type FROM {$prefix}categories")->fetchAll();
    foreach ($rows as $r) {
    ...
    $prodStmt = $db->prepare("
        INSERT INTO {$prefix}products (name, description, price, category, type, image_url)
    ...
    $sizeStmt = $db->prepare("INSERT INTO {$prefix}gelato_sizes (id, name, balls, price) VALUES (:id, :name, :balls, :price)");
    $sizes = [
    ...
    $boxStmt = $db->prepare("INSERT INTO {$prefix}chocolate_boxes (id, name, units, price) VALUES (:id, :name, :units, :price)");
    $boxes = [
    ...
    $topStmt = $db->prepare("INSERT INTO {$prefix}toppings (id, name, price) VALUES (:id, :name, :price)");
    $toppings = [
    ...
    jsonResponse([
        'success' => true,
        'message' => 'Setup Orlandia concluido com sucesso!',
        'details' => $results,
    ]);
